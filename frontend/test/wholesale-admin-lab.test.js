...
const completeLot = {
  id: "demo-lot-test-a", status: "draft", title: "Example OEM 32GB DDR4 ECC RDIMM", brand: "Example OEM", mpn: "DEMO-TEST-A",
  generation: "DDR4", formFactor: "RDIMM", capacityLabel: "32GB", speedLabel: "3200 MT/s", rank: "2Rx4",
  condition: "Server Pull — Tested", testStatus: "MemTest verified", warranty: "90 Days", quantityAvailable: 20,
  minimumOrderQuantity: 4, orderIncrement: 2, shipFrom: "Toronto, Canada",
};

test("missing local state seeds two published examples and one private draft", () => {
  const storage = fakeStorage(); const state = readWholesaleDemoState(storage);
  assert.equal(state.error, null); assert.equal(state.seeded, true); assert.equal(state.lots.length, 3);
...
test("invalid or unsupported local state fails closed with no customer-visible lots", () => {
  ...
  for (const storage of [invalid, wrongVersion]) {
    ...
    assert.deepEqual(state.lots, []); assert.match(state.error, /could not be read/i);
...
test("valid-schema but incomplete published data remains hidden from customers", () => {
  const storage = fakeStorage([[WHOLESALE_DEMO_STORAGE_KEY, JSON.stringify({ schemaVersion: WHOLESALE_DEMO_SCHEMA_VERSION, lots: [{ id: "demo-lot-tampered", status: "published", quantityAvailable: 12, minimumOrderQuantity: 4 }] })]]);
  const state = readWholesaleDemoState(storage);
  assert.equal(state.error, null); assert.equal(state.lots.length, 1);
  assert.deepEqual(publishedWholesaleDemoLots(state.lots), []);
});

test("the demo sanitizer strips retail fields and forces local quote-only identity", () => {
  const lot = sanitizeWholesaleDemoLot({ ...completeLot, price: 99, stockQuantity: 500, stripePriceId: "price_forbidden", visibility: "public", isDemo: false, quoteOnly: false });
  assert.equal(lot.visibility, "local-demo"); assert.equal(lot.isDemo, true); assert.equal(lot.quoteOnly, true);
  for (const retailField of ["price", "stockQuantity", "stripePriceId"]) assert.equal(retailField in lot, false);
});
...
test("exact upsert, publish visibility, round-trip, and removal remain provider-separated", () => {
  const storage = fakeStorage(); const initial = readWholesaleDemoState(storage).lots;
  ...
  assert.equal(withNew[0].id, completeLot.id); assert.equal(publishedWholesaleDemoLots(withNew).length, 3);
...
test("a failed browser-local write cannot replace the previous serialized state", () => {
  ...
  const storage = { getItem() { return original; }, setItem() { throw new Error("quota exceeded"); } };
...
test("restoring examples replaces only seeded IDs and preserves custom lots", () => {
  const storage = fakeStorage(); const seeded = readWholesaleDemoState(storage).lots;
  ...
  assert.equal(restored.length, 4); assert.equal(restored.some((lot) => lot.id === custom.id), true);
});

test("both local views are development-only and contain no retail or production mutation path", async () => {
  const [app, admin, customer, adminCss] = await Promise.all([
    readFile(new URL("../src/App.jsx", import.meta.url), "utf8"), readFile(new URL("../src/pages/WholesaleAdminLab.jsx", import.meta.url), "utf8"),
    readFile(new URL("../src/pages/WholesaleLab.jsx", import.meta.url), "utf8"), readFile(new URL("../src/pages/wholesale-admin-lab.css", import.meta.url), "utf8"),
  ]);
  ...
  for (const source of [admin, customer]) assert.doesNotMatch(source, /productsApi|adminApi|\/api\/products|stockQuantity|cartApi|checkoutApi|stripePrice/i);
